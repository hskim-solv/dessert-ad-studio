...
| Medium | Async job evidence started as mostly happy-path smoke. | Accepted and first live failure gate closed | Burst jobs, failure state, queue enqueue failure, duplicate polling, worker startup wait, K8s async smoke, live worker outage/restore, explicit retry/timeout/cancel non-support, and scoped multi-worker policy are covered. Exactly-once processing and worker affinity remain unclaimed. |
...
| P6 | Async reliability matrix. | First live failure gate complete: burst submit, workflow failure state, queue enqueue failure, duplicate polling, worker startup wait, K8s async smoke, live worker outage/restore, explicit retry/timeout/cancel non-support, and scoped multi-worker policy passed. Exactly-once processing and worker affinity remain unclaimed. |
