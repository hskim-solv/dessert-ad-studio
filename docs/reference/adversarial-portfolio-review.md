...
| P3 | Provider-quality image-edit gate. | Paid `gpt-image-2` + `quality=medium` run over 3 public samples is complete and failed: ROI preservation checks passed, but latency, text-contamination, and cost guard failed. Provider-quality image editing remains unproven. |
