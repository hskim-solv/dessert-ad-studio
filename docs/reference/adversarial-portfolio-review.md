...
| P3 | Provider-quality image-edit gate. | Paid `gpt-image-2` + `quality=medium` run over 3 public samples is complete and failed: ROI preservation checks passed, but latency, text-contamination, and cost guard failed. A later one-sample canary reached the API and stayed under budget, but still failed latency and text-contamination proxy checks; manual local review found no visible text, so proxy calibration is now the main remediation before another full paid gate. Provider-quality image editing remains unproven. |
| P4 | Real evaluation pack. | First product-like gate complete: 30 deterministic workflow scenarios with reproducible summary JSON. Offline visual proxy gate complete for 6 committed banners. Human rubric, real samples, and provider-quality visual statistics remain pending. |
