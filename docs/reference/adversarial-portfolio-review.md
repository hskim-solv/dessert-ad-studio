...
| Medium | Async job evidence started as mostly happy-path smoke. | Accepted and first live failure gate closed | Burst jobs, failure state, queue enqueue failure, duplicate polling, worker startup wait, K8s async smoke, and live worker outage/restore are covered. Retry, timeout/dead-letter, cancellation, and multi-worker behavior remain unclaimed. |
...
| P6 | Async reliability matrix. | First live failure gate complete: burst submit, workflow failure state, queue enqueue failure, duplicate polling, worker startup wait, K8s async smoke, and live worker outage/restore passed. Retry/timeout/cancel and multi-worker failure behavior remain pending. |
