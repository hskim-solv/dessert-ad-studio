...
- Full LangGraph production orchestration. The first offline graph, SSE, local
  ...
  gates plus local approval API and reviewer approval UI first gates are
  complete, but post-approval worker resume, Postgres or production storage
  policy, production trace retention policy, and live provider token/cost
...
trace/run-metrics/reviewer-approval gates to post-approval worker resume,
production replay retention, production MCP transport/auth, and provider-backed
token/cost telemetry while preserving the current evidence base. Paid provider-quality image-edit
