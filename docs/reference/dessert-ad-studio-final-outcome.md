...
- Live/production tool suite. Local web search, allowlisted SQLite SQL with
  ...
  production MCP auth provider selection, and remote client transport/auth
...
| Agent tool suite | Decided: local deterministic tool suite first, FastMCP local smoke | See ADR 0017. Live web search runtime policy, local SQL runtime policy, production DB access/audit policy, and loopback-only MCP transport/auth boundary first gates are complete; live web search provider smoke, credentialed production DB smoke, production MCP auth provider, and remote client smoke require separate runtime/security evidence. |
