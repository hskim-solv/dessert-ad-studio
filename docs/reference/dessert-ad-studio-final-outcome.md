...
| M4 Real product analysis | Replace mock product analysis with a real VLM-backed analyzer while preserving redaction policy. | Partial: OpenAI Responses Vision adapter, ADR, no-network tests, env/compose wiring. Remaining: live smoke, eval samples, product-preservation checklist pass rate >= 80%. |
