...
- First Agentic RAG retention boundary gate: ADR 0018 adopts redacted replay
  ...
  cross-process resume store, production approval audit retention, and external
  trace payload retention require explicit user decision. Evidence is recorded in
...
- Full LangGraph production orchestration. The first offline graph, SSE, local
  ...
  resume, an approved production storage implementation, deployment-specific
  external trace retention, and live provider token/cost telemetry remain
  pending.
...
- Live/production tool suite. Local web search, allowlisted SQLite SQL with
  ...
  credentialed production DB connection/audit-retention smoke, production MCP
  auth provider selection, and remote client transport/auth smoke remain
  pending.
...
| Image quality | Product-preservation checklist pass rate >= 80%; Korean overlay rendering failures 0. Deterministic public-sample preservation first gate: pass rate 1.00, minimum top-region pixel match 1.00. Offline visual proxy gate passes 6 committed banners and includes a blank-image negative regression. Paid OpenAI image-edit gates failed and are documented as model-quality evidence, not hidden. The latest `gpt-image-2`/`medium` canary passed ROI color/hash/edge preservation, script cost guard, and post-calibration text-contamination checks but failed latency. Paid provider-quality image editing remains unproven until latency strategy is resolved and a later gate passes. |
...
| Agent tool suite | Decided: local deterministic tool suite first, FastMCP local smoke | See ADR 0017. Local SQL runtime policy, production DB access/audit policy, and loopback-only MCP transport/auth boundary first gates are complete; live web search, credentialed production DB smoke, production MCP auth provider, and remote client smoke require separate runtime/security evidence. |
...
portfolio gap is latency strategy/remediation and human/provider
generated-asset quality review. A one-sample `--sample-slug` canary is
available as the narrow paid gate before any full three-sample iteration, but
the next paid spend should be preceded by an explicit latency strategy.
