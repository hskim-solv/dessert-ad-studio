...
- MCP는 `mcp_servers/dessert_ad_studio_server.py` FastMCP server와 `mcp`
  ...
  실행한다. 장기 실행 transport/auth smoke는 production boundary가 정해질 때
  별도로 추가한다.
...
- 이 선택으로 감수하는 것:
  ...
  - MCP server는 local import/tool-call smoke까지만 proven이다. 장기 실행
    transport, auth, remote client contract는 별도 evidence가 필요하다.
- 재평가 트리거:
