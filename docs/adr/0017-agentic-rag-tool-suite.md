...
이번 결정의 목표는 외부 네트워크, paid API, production DB 없이 도구 orchestration
증거를 먼저 만드는 것이다. 실제 live web search와 production SQL은 비용/보안/retention
정책이 필요하므로 기본 CI에 넣지 않는다.
...
| 기준 | 후보 A: local deterministic tool suite + optional FastMCP scaffold | 후보 B: live web/production SQL/MCP를 즉시 도입 | 후보 C: 문서만 pending 유지 |
|---|---|---|---|
...
| 보안 경계 | 높음. query text/raw request를 output에 남기지 않고 SQL은 query_id allowlist만 허용한다. | 낮음에서 중간. live connector별 보안 검토가 필요하다. | 높음. 실행 surface가 없기 때문이다. |
...
- `sql_query`는 in-memory SQLite와 query_id allowlist만 허용한다.
- `internal_api`는 in-process `preview_generation_policy` contract로 시작한다.
- MCP는 `mcp_servers/dessert_ad_studio_server.py` FastMCP scaffold와 optional
  `mcp` dependency extra로 둔다. 기본 CI는 아직 MCP package execution을 하지 않는다.

## 결과 및 재평가 조건 (Consequences)

- 이 선택으로 감수하는 것:
  - web search는 live search가 아니라 local snapshot이다.
  - SQL query는 production DB가 아니라 allowlisted SQLite proof다.
  - MCP server는 scaffold이며 package execution smoke는 아직 proven claim이 아니다.
- 재평가 트리거:
  - MCP tool server를 portfolio demo에 포함해야 할 때.
  - production DB query/audit policy가 정해질 때.
