...
production storage or cross-process resume claim is made.
...
- Durable raw request storage, cross-process resume, production approval audit
  retention, and external trace payload retention require a separate user
  decision.
...
- durable cross-process resume: `pending_user_decision`
...
- It intentionally blocks durable raw request storage until storage location,
  ...
  decided.
- Durable cross-process resume remains pending.
