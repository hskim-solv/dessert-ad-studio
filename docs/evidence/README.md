...
Latest local regression snapshot: `251 passed`.
...
| Agentic RAG eval and guardrails | [`agentic-rag-eval-guardrail.md`](agentic-rag-eval-guardrail.md), [`agentic-rag-eval-guardrail-summary.json`](agentic-rag-eval-guardrail-summary.json) | Local Ragas/promptfoo-compatible first gate passed: 13 golden cases, faithfulness/answer relevancy/context precision/context recall all 1.00, prompt-injection route blocked before worker, tool allowlist/budget passed, raw inputs absent | `.venv/bin/python scripts/agentic_rag_eval_guardrail.py --date 2026-06-17 --output docs/evidence/agentic-rag-eval-guardrail-summary.json` |
