...
| Observability | JSONL generation logs, `/metrics` | OpenTelemetry, Langfuse, Phoenix, LangSmith, Prometheus/Grafana | Add OpenTelemetry + Langfuse or Phoenix after workflow exists | Agent trace evidence is a high-value hiring signal. Keep JSONL as fallback/local audit log. |
...
| Image pipeline | OpenAI reference edit; FLUX.2 text-to-image only | rembg, SAM/SAM2, IP-Adapter, ControlNet, ComfyUI, Diffusers, PIL composition | Add product segmentation/compositing before more model serving | The business-critical quality is product preservation. Product cutout/composite plus Korean overlay is more valuable than pure t2i novelty. |
